# TODO: I should probably rename the parameters to "parameter_hat" much
...
calculate_coverage <- function(
    ...
    alpha <- 1 - prob
    var_name <- rlang::enquo(param_hat)

    coverage_intervals <- reshaped_posterior |>
        dplyr::group_by(id) |>
        dplyr::summarize(
            lower = quantile(!!var_name, alpha / 2),
            upper = quantile(!!var_name, 1 - alpha / 2)
        )

    covered <- (true_values >= coverage_intervals[, "lower"]) &
        (true_values <= coverage_intervals[, "upper"])
