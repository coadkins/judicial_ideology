...
(3) $\mu_{\beta_B} > 0$ or $\beta_j < 0$. Sign invariance for some case $j$ on each factor $\mathcal{D}$ (in this case there is only one dimension). One approach to this constraint is to directly constrain the sign of some $
beta$ to be positive or negative. In practice, it can be difficult to specify the constraint without causing mcmc chains to "bunch" near the constraint. 

Instead, I employ employ "Rotation Sign Permutation" after sampling to harmonize draws across MCMC chains (Papastamoulis et. al 2022). For IRT models with only one dimension, this procedure forces the mean draw for each chain to share the same sign; chains that do not meet this constraint have their draws multiplied by -1. 
...
I validate the appropriateness of the model using simulated data that matches the distribution of ability scores and outcomes predicted by my theory.

In one MCMC run, the group level 95% credible intervals cover the true parameters values for 95% of cases. In this figure, the gray box plots represent the 95% credible interval for estimates of $\mu_theta$ for each group. The red and blue colored boxes represent the middle quartiles of the distribution of $\theta$ for each group, with the black horizontal line representing the true group mean. 

![Figure 1](../good_validation.png)

After RSP, the trace plots suggest that the model is well identified:

![Figure 2](../good_trace.png)
...
- Imposing sign constraints on some $\alpha_j$ and/or $\beta_j$. The appeal in this context is that sign constraints on case categories are easy to justify substantively. In practice, a hard constraint often triggers some chains to fail to explore the parameter space. Likewise, it is difficult to express a "soft" constraint in terms of priors that avoids chain "bunching." 
