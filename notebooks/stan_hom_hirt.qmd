...
- $\alpha_j$ is the case-specific "difficulty" parameter. We can think of this as the how likely a Republican judge is to vote for the defendant
- $\beta_j$ is the case-specific "discrimination" parameter. 
- $\theta_i$ is the judge-specific "ability" parameter in my case, this is the ideology score. 

- The link function is an "Ordinal Logistic" Function with 2 cutpoints corresponding to three possible ordered outcomes.

$$
\text{OrderedLogistic}(k | \eta_ij, c) =
\begin{cases}
    1 - \text{logit}^{-1}(\eta - c_1) & \text{if } k = 1, \\
    \text{logit}^{-1}(\eta - c_1) - \text{logit}^{-1}(\eta - c_2) & \text{if } k = 2, \\
    \text{logit}^{-1}(\eta - c_2) - \text{logit}^{-1}(\eta - c_3) & \text{if } k = 3, \\
    \text{logit}^{-1}(\eta - c_3) & \text{if } k = 4.
\end{cases}
$$
...
$\eta$ has a very wide prior of $\mathcal{N} (0, 10)$.
